...
public final class TypeVariableName extends TypeName {
	 ...
	 * {@code Enum<E extends Enum<E>>}. When we encounter such a thing, we will make
	 * a TypeVariableName without bounds and add that to the {@code typeVariables}
	 * map before looking up the bounds. Then if we encounter this TypeVariable
	 * again while constructing the bounds, we can just return it from the map. And,
	 * the code that put the entry in {@code variables} will make sure that the
	...
	static TypeVariableName get(TypeVariable mirror, Map<TypeParameterElement, TypeVariableName> typeVariables) {
		...
		if (typeVariableName == null) {
			// Since the bounds field is public, we need to make it an unmodifiableList. But
			// we control
