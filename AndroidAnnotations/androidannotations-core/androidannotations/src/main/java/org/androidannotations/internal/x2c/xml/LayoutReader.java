...
import org.androidannotations.internal.x2c.view.View;
...
public class LayoutReader {
	...
	private class XmlHandler extends DefaultHandler {
		...
		public void endElement(String uri, String localName, String qName) throws SAXException {
			...
			if (mStack.size() > 0) {
				...
				if (mStack.size() == 0) {
					...
					LayoutWriter writer = new LayoutWriter(stringBuilder.toString(), mFiler, mName, mPackageName, Util.getLayoutCategory(mFile), mLayoutName, mRootView.getImports());
		...
		private View createView(String name, Attributes attributes) {
			if ("layout".equals(name) || "data".equals(name) || "variable".equals(name) || "import".equals(name)) {
				isDataBinding = true;
				return null;
			}
