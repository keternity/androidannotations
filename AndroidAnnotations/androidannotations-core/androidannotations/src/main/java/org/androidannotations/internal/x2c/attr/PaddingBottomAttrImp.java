...
public class PaddingBottomAttrImp extends AutoAttr {
	...
	protected String execute(String view, String dataVal) {
		return String.format("%s.setPadding(%s,%s,%s,%s);", view, String.format("%s.getPaddingLeft()", view), String.format("%s.getPaddingTop()", view), String.format("%s.getPaddingRight()", view),
				dataVal);
	}
}
