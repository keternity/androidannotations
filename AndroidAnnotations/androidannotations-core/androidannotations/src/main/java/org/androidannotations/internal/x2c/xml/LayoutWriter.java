...
public class LayoutWriter {
	...
	public LayoutWriter(String methodSpec, Filer filer, String javaName, String pkgName, String layoutSort, String layoutName, Set<String> imports) {
	...
	public String write() {
		MethodSpec methodSpec = MethodSpec.methodBuilder("createView").addParameter(ClassName.get("android.content", "Context"), "ctx").addStatement(isMerge ? "return null" : mMethodSpec)
				.returns(ClassName.get("android.view", "View")).addAnnotation(Override.class).addModifiers(Modifier.PUBLIC).build();
		MethodSpec methodSpecEx = MethodSpec.methodBuilder("createView").addParameter(ClassName.get("android.view", "ViewGroup"), "viewGroup").addStatement(isMerge ? mMethodSpec : "return viewGroup")
				.returns(ClassName.get("android.view", "View")).addAnnotation(Override.class).addModifiers(Modifier.PUBLIC).build();

		TypeSpec typeSpec = TypeSpec.classBuilder(mName).addMethod(methodSpec).addMethod(methodSpecEx).addSuperinterface(IViewCreator.class).addModifiers(Modifier.PUBLIC)
				.addJavadoc(String.format("WARN!!! dont edit this file\ntranslate from {@link  %s.R.layout.%s}", mPkgName, mLayoutName)).build();
		...
		JavaFile javaFile = JavaFile.builder(pkgName, typeSpec).addImports(mImports).build();
		try {
