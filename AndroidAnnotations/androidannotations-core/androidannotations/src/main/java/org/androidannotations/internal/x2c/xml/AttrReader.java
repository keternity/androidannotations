...
public class AttrReader {
	private Map<String, Attr> mAttrs = new HashMap<>();
	private File mFile;
	private SAXParser mParser;
	private Map<String, Func> mAttr2Funcs;
	/**
	 * attName like "progress",but when we used in layout is "app:progress" so we
	 * need a map
	 */
	private Map<String, String> mAttrNameMap = new HashMap<>();

	public AttrReader(File file, Map<String, Func> attr2Func) {
	...
	private File getValueFile(File root) {
		...
		File valueDir = new File(root.getAbsolutePath() + sep + "build" + sep + "intermediates" + sep + "incremental");
		if (valueDir != null) {
		...
		if (valueFile == null) {
			valueFile = new File(root.getAbsolutePath() + sep + "src" + sep + "main" + sep + "res" + sep + "values" + sep + "attrs.xml");
		}

		return valueFile;
	}

	public Map<String, Attr> parse() {
		try {
	...
	private class AttrHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
			...
			} else if ("enum".equals(qName) || "flag".equals(qName)) {
				if (mAttr != null) {
